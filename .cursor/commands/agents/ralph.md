...
1. **Input:** An idea or a PRD (JSON/Markdown or already in OpenThrottle). If it's rough, turn it into a **plan** and **tasks** in OpenThrottle — OpenThrottle MCP `create_plan` / `create_task` per `../../rules/commands/openthrottle.mdc`. For a strict, hyper-detailed PRD, ensure plan/tasks in OpenThrottle match the PRD; required vs optional vs inferred attributes are defined in `../../../databases/README.md`.
...
- Follow `../../rules/commands/agents.mdc` and `../../rules/commands/github.mdc`.
...
- **OpenThrottle:** What it is, how to interact, MCP tools, and Cursor commands: `../../commands/openthrottle.mdc`. Schema and PRD attribute mapping (required / inferred / optional): `../../../databases/README.md`.
