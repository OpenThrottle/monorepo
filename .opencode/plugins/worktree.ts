...
const Result = {
  err: <E>(error: E): ErrResult<E> => ({ error, ok: false }),
...
const worktreeConfigSchema = z.object({
  ...
  sync: z
    ...
    .default(() => ({ copyFiles: [], exclude: [], symlinkDirs: [] })),
...
function resolveStableLaunchBinaryPath(
  ...
  if (isPathLikeCommand(ocxBin)) {
    const resolvedPath = path.isAbsolute(ocxBin)
      ? ocxBin
      : path.resolve(baseDirectory, ocxBin);
  ...
  if (!resolvedFromPath) {
    return Result.err(
      `Configured OCX binary "${ocxBin}" is not available in PATH.`,
    );
  }
...
async function pathPointsToLaunchableBinary(
  absolutePath: string,
): Promise<boolean> {
  try {
...
async function ensureLaunchContextExecutable(
  ...
  const resolveExecutable =
    options.resolveExecutable ?? ((command: string) => Bun.which(command));
  const pathExists = options.pathExists ?? pathPointsToLaunchableBinary;
  const resolvedPathResult = resolveStableLaunchBinaryPath(
    ocxBin,
    baseDirectory,
    resolveExecutable,
  );
...
async function validateOcxProfileAvailability(
  ...
  try {
    const proc = Bun.spawn(
      [ocxBin, 'profile', 'show', profile, '--global', '--json'],
      {
        stderr: 'pipe',
        stdout: 'pipe',
      },
...
async function finalizeWorktreeLaunch(
  ...
  if (!terminalResult.success) {
    await deleteForkedSessionFn(options.forkedSessionId).catch(
      (cleanupError) => {
        options.log.warn(
...
async function forkWithContext(
  ...
  try {
    ...
    await rm(destWorkspaceDir, { force: true, recursive: true }).catch((e) => {
      client.app
        .log({
          body: {
            level: 'error',
    ...
    await rm(destDelegationsDir, { force: true, recursive: true }).catch(
      (e) => {
        client.app
          .log({
            body: {
  ...
  return { delegationsCopied, forkedSession, planCopied, rootSessionId };
}
...
const WorktreePlugin: Plugin = async (ctx) => {
  ...
  const log = {
    debug: (msg: string) =>
      client.app
        .log({ body: { level: 'debug', message: msg, service: 'worktree' } })
        .catch(() => {}),
    error: (msg: string) =>
      client.app
        .log({ body: { level: 'error', message: msg, service: 'worktree' } })
        .catch(() => {}),
    info: (msg: string) =>
      client.app
        .log({ body: { level: 'info', message: msg, service: 'worktree' } })
        .catch(() => {}),
    warn: (msg: string) =>
      client.app
        .log({ body: { level: 'warn', message: msg, service: 'worktree' } })
        .catch(() => {}),
  };
    ...
    tool: {
      worktree_create: tool({
        ...
        async execute(args, toolCtx) {
          ...
          const persistedLaunchMetadata =
            toPersistedLaunchMetadata(activeLaunchContext);
          const launchArgv = buildSessionLaunchArgv(
            forkedSession.id,
            persistedLaunchMetadata,
          );
          const serializedLaunchMetadata = serializePersistedLaunchMetadata(
            persistedLaunchMetadata,
          );
      ...
      worktree_delete: tool({
        ...
        async execute(_args, toolCtx) {
          ...
          setPendingDelete(
            database,
            { branch: session.branch, path: session.path },
            client,
          );
