...
function ensureSessionLaunchMetadataColumns(db: Database): void {
  ...
  const sessionColumns = new Set(
    tableInfo.map((column) => column.name).filter(Boolean),
  );
  ...
  if (!sessionColumns.has('launch_mode')) {
    addSessionColumn(
      db,
      'launch_mode',
      'ALTER TABLE sessions ADD COLUMN launch_mode TEXT',
    );
  }
  ...
  if (!sessionColumns.has('ocx_bin')) {
    addSessionColumn(
      db,
      'ocx_bin',
      'ALTER TABLE sessions ADD COLUMN ocx_bin TEXT',
    );
  }
}
...
export function addSession(db: Database, session: SessionInput): void {
  ...
  const serializedLaunchMetadata =
    serializePersistedLaunchMetadata(launchMetadata);
...
export function getSession(db: Database, sessionId: string): Session | null {
  ...
  const row = stmt.get({ $id: sessionId }) as Record<
    string,
    string | null
  > | null;
  if (!row) return null;
