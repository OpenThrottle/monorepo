...
function requireNonEmptyField(
  ...
  throw new Error(
    `${source} requires ${fieldName} to be set to a non-empty value`,
  );
}
...
export function parsePersistedLaunchMetadata(
  ...
  if (launchMode !== 'ocx') {
    throw new Error(
      `Invalid persisted launch metadata: unsupported launchMode "${launchMode}"`,
    );
  }
...
export function serializePersistedLaunchMetadata(
  metadata: PersistedLaunchMetadata,
): {
