...
export function buildBatchCommandFromArgv(argv?: string[]): string | undefined {
  ...
  return normalizedArgv
    .map((arg) => `"${escapeBatch(arg).replace(/"/g, '""')}"`)
    .join(' ');
}
...
type RunCmuxCommand = (
  args: string[],
) => CmuxCommandResult | Promise<CmuxCommandResult>;
...
export async function openCmuxTerminalWithState(
  ...
  if (!canUseCmuxWorkflow(env, resolveExecutable, cmuxCommand)) {
    return {
      hasStateMutation: false,
      terminalResult: {
        error: 'cmux environment not available',
        success: false,
      },
    };
  }
  ...
  const runCmuxCommand: RunCmuxCommand =
    options?.runCmuxCommand ??
    ((args) => runCmuxCommandWithBun(cmuxCommand, args));
  ...
  return { hasStateMutation, terminalResult: { success: true } };
}
...
export async function openLinuxTerminal(
  ...
  try {
    ...
    const tryTerminal = async (
      ...
      if (check.exitCode !== 0) {
        return { success: false, tried: false };
      }
      ...
      try {
        ...
        proc.unref();
        return { success: true, tried: true };
      } catch {
        return { success: false, tried: true };
      }
    };
    ...
    if (currentTerminal) {
      ...
      switch (currentTerminal) {
        ...
        case 'warp': {
          ...
          const configContent = buildWarpLaunchConfigYaml(
            configName,
            cwd,
            configPath,
            command,
        ...
        default:
          result = { success: false, tried: false };
      }
    ...
    const modernTerminals: Array<{ name: string; args: string[] }> = [
      {
        args: ['kitty', '--directory', cwd, '-e', 'bash', launchScriptPath],
        name: 'kitty',
      },
      {
        args: [
      ...
      { args: ['ghostty', '-e', 'bash', launchScriptPath], name: 'ghostty' },
      {
        args: ['foot', '--working-directory', cwd, 'bash', launchScriptPath],
        name: 'foot',
      },
    ];
    ...
    const deTerminals: Array<{ name: string; args: string[] }> = [
      ...
      {
        args: ['konsole', '--workdir', cwd, '-e', 'bash', launchScriptPath],
        name: 'konsole',
      },
      {
        args: [
    ...
    scriptPath = null;
    warpConfigPath = null;
    return { error: 'No terminal emulator found', success: false };
  } catch (error) {
    await cleanupFile(scriptPath);
    await cleanupFile(warpConfigPath);
    return {
...
export async function openTerminal(
  ...
  if (terminalType === 'cmux') {
    const cmuxResult = await (
      options?.openCmuxTerminalWithState ?? openCmuxTerminalWithState
    )(cwd, argv);
...
async function openPlatformTerminal(
  cwd: string,
  argv?: string[],
): Promise<TerminalResult> {
