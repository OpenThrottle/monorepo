...
- **Prettier** via lint-staged and `editor.formatOnSave`, or repo-wide with `pnpm nx run monorepo:format:fix` / `pnpm nx run monorepo:format:check`. Config is the single `prettierConfig` from `@tools/dotfiles`.
