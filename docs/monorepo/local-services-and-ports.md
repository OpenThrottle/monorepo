...
| **Ollama**                 | **11434**           | Local LLM/embedding server. Used by `openthrottle:import`, LangChain, openthrottle-server, and other agents. | `OLLAMA_BASE_URL` default `http://localhost:11434`. See `scripts/ollama.ts (pnpm run ollama:pull)`, `databases/README.md`, `.env.default`.               |
...
- **Postgres (OpenThrottle)** — `localhost:6010` (from `applications/openthrottle-server/.env.default`: `POSTGRES_PORT`). Typically not exposed through Caddy; used by openthrottle-server and OpenThrottle tooling.
...
- Plan: _Expose local services with Caddy and host Ollama for agents_ (OpenThrottle).
- Caddyfile docs: <https://caddyserver.com/docs/caddyfile>.
- Ollama in this repo: `docs/monorepo/Ollama.md`, `scripts/ollama.ts (pnpm run ollama:pull)`, `databases/README.md`, `AGENTS.md` (§ OLLAMA_BASE_URL).
