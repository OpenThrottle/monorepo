...
Ten standalone audit prompts live in [`.agents/prompts/`](../../.agents/prompts/). Each one sweeps a single dimension of repository health, is **read-only on source code**, and has exactly one side effect: it files a single OpenThrottle plan with prioritized, self-contained tasks. Nothing merges without a human starting that plan.

They are invoked two ways:

```bash
# Ad-hoc, via the Ralph CLI (layer-1 prompt delivery)
pnpm exec workflow-ralph --prompt-file .agents/prompts/Job_DeadCode.md
```
...
- **Mechanical sweeps** — `WorkInFlight`, `TestHealth`, `DeadCode`, `DocDrift`, `RulesConformance` — run on `sonnet`. Their findings come from tool output plus deterministic reading, so intelligence beyond that tier buys nothing and these are the most frequent runs.
...
Register against a checkout where the job's commands actually resolve — `cwd` defaults to `WORKSPACE_ROOT`. Enable one job first (`Job_WorkInFlight` is the cheapest and the most immediately useful), confirm it files exactly one plan and edits nothing, then enable the rest.
