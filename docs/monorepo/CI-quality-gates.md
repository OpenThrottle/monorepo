...
| **P4** | Full monorepo `test` (all apps/packages) | — (commented `parallelize-tasks` matrix) | `scripts/parallelize-tasks.ts` + `nx affected --target=test` | [visormatt](https://github.com/visormatt) | — | **Off** — track via OT plan *Improve test coverage and CI test execution* |
