...
| **P2**   | Vitest (all affected projects)                | `build` → NX `<target>` affected step (per shard)                               | `nx affected --exclude="*,$selector" --target=test`                                                                              | [visormatt](https://github.com/visormatt) | Yes (affected)  | **On** (full affected, split across 3 shards)                                                                             |
...
| **P0**   | Sharded affected `lint`/`typecheck`/`test`    | `build` → 3-leg matrix (`jobIndex: [1, 2, 3]`, `fail-fast: false`)              | `scripts/parallelize-tasks.ts` → `nx affected --exclude="*,$selector"`                                                           | [visormatt](https://github.com/visormatt) | Yes (affected)  | **On** — OT plan `b19377d1`; sizing rationale in [ci-cost.md](./ci-cost.md) § CI sharding                                 |
