...
As-built status (Plan-Id `1bee2916-…`). Tasks 1–10 landed complete with tests.
Tasks 1–7 + 10 shipped in PR #190; tasks 9 and 8 followed on branch
`feat/lifecycle-hooks-developer-ui` (see rows below).
...
| 8   | ✅ done | **Scope corrected — the runner was already wired** (`queues/job-run-hooks/execute-plan-job-run-hooks.ts` + `queues/plan-lifecycle-hooks/` dispatcher, invoked from `plans.processor.ts`; the orchestrator already fires `beforeEach`/`afterEach` per task). **8a:** `PlanEnqueueService` projects plan-level skill hook-tasks (`projectHookTasksToJobRunHookEntries`) and unions them into the enqueue `JobRunHooksConfig` (`resolveJobRunHooksForEnqueue`, dedup by phase+skillPath) — so materialized plan-level skill hooks fire via the runner (beforeAll/afterAll + beforeEach/afterEach each-expansion). **8b:** the orchestrator loop is hook-aware (`isRunnableRalphTask`/`isRunnerExecutedHookTask`) — runner-executed plan-level skill hook-tasks are excluded from agent task-selection (no double execution) while task-level + template hooks still run as materialized tasks. **Limits:** spawn CLI path has no per-task firing; `taskOutcome:'failed'` + task-centric single-task runs unchanged |
| 9   | ✅ done | `HookTaskList` presentational component + tests (PR #190); then plan-level (Details tab) and task-level (task-detail route) nested hooks wired: `HookTask` fragment + nested `beforeHooks`/`afterHooks` + `hookRole` + `addHook`/`detachHook` mutations, `PlanLifecycleHooks`/`AddHookDialog`/`PlanLifecycleHooksSection`, materialized hook-tasks filtered out of the task list+board (`hook-tasks` util). Exposed hook discriminators on GraphQL `TaskObject` (additive)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 10  | ✅ done | this section; cross-project parity verified (no schema/codegen drift)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |

### Open questions — resolutions

Q1 (each materialization): **resolved — virtual `each`.** Plan-level `each` skill
hooks are not materialized per task; they project into `beforeEach`/`afterEach`
`JobRunHookEntry`s and fire around each task via the lifecycle dispatcher's
`runTask`. Q2 (`jobRunHooksJson` fate): still writable; no D1 cutover — task-8a
unions materialized-derived hooks with `jobRunHooksJson` (additive) rather than
replacing it. Q3 (`first`/`last` aliasing): **adopted** (task 4 band placement).
Q4 (limits): not yet enforced on materialized hooks — a service-layer cap is a
follow-up. Q5 (`each` scoping): honored via `JobRunHookConditions`
(`taskCategories`/`taskStatuses`) on the projected entries, filtered in the
dispatcher per task.
