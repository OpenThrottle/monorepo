...
- **Task wave encoding:** [openthrottle/task-wave-encoding.md](./openthrottle/task-wave-encoding.md) — how a plan says two tasks may run at once, why it is a nullable `wave` integer rather than a dependency graph, and why an unpopulated plan stays sequential.
- **Lane-parallelism feasibility audit:** [openthrottle/lane-parallelism-feasibility-audit.md](./openthrottle/lane-parallelism-feasibility-audit.md) — how much real cross-task concurrency historical plans show, the pre-registered go/no-go threshold, and why the answer is NO.
