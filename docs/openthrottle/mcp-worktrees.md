...
Using this repo with custom MCPs (mcp-developer, docs-mcp) across git worktrees can cause Cursor to treat multiple instances as the same server when they share the same name. This doc summarizes how Cursor keys MCP servers, what we did to make mcp-developer and docs-mcp worktree-aware, and how other MCPs in `.cursor/mcp.json` behave.

## How Cursor keys MCP servers

- **Config:** Cursor reads `.cursor/mcp.json` per workspace. Each workspace (e.g. each worktree) has its own file; the **key** in `mcpServers` (e.g. `mcp-developer`, `docs-mcp`) is the server identifier Cursor uses.
...
- **Per-workspace config:** Cursor does not document using path or workspace in the key; the key is a static string in `mcp.json`. So to get distinct identity per worktree we make the **server’s advertised name** worktree-aware (see below). Optionally, you can use different keys per worktree (e.g. generate `mcp.json` with `mcp-developer-worktree-one`, `mcp-developer-worktree-two`) for full isolation.

## Worktree-aware mcp-developer and docs-mcp
