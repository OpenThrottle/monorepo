...
| Spawn Ralph (`enqueuePlanRun` → nested `workflow-ralph`; default GraphQL transport, or `WORKFLOW_RALPH_TRANSPORT=postgres-direct`) | task/plan via Ralph helpers            | GraphQL transport: via resolvers; postgres-direct: **No** | GraphQL transport: via resolvers; postgres-direct: **No**               |
| MCP / GraphQL clients (`update_plan`, `update_task`)                                                                               | task/plan                              | via resolvers above                                       | via resolvers above                                                     |

**Known gap (by design):** nested spawn Ralph with postgres-direct transport writes OpenThrottle directly; the developer app will not receive socket events until the next GraphQL-driven change or a full revalidate. Orchestrator, MCP, and GraphQL API paths stay in sync via resolvers.

**Manual plan/task edits:** Legacy one-off CLI scripts under `tools/workflows/scripts/update-*.ts` were removed. Use **openthrottle-mcp** `update_plan` / `update_task` (or GraphQL `updatePlan` / `updateTask`) so status changes emit `plan.status_changed` / `task.status_changed` on the WebSocket.
