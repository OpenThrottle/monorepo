...
This document breaks down the implementation of the Passport local strategy for authentication (OpenThrottle plan `84bf03c3-81d8-43e1-8b2d-0a2026a1e798`). Reference: [openthrottle-server-auth.md](./openthrottle-server-auth.md).
...
- **Target:** Validate credentials against OpenThrottle `users` table via a Passport local strategy; sign JWT only for validated users with `sub: user.id`.
- **Stack:** openthrottle-server (NestJS, GraphQL), `@openthrottle/nestjs-repositories` (UsersService, User entity), `@openthrottle/nestjs-auth` (JWT strategy, guards). OpenThrottle DB already has a `users` table (migration 026) with `id`, `email` (nullable), `github_username`; no password column yet.

## Detailed Tasks (for OpenThrottle)

Add these as child tasks of plan `84bf03c3-81d8-43e1-8b2d-0a2026a1e798` via OpenThrottle MCP (openthrottle-mcp) `create_task`.

1. **OpenThrottle DB: Add password_hash and email uniqueness for local auth**
   - Add migration in `databases/migrations/`: column `password_hash` (TEXT, nullable so existing users without passwords remain valid). Add unique index on `email` where email IS NOT NULL so local-auth users have a unique email. Document in OpenThrottle README.
...
8. **Update auth documentation**
   - Update `docs/openthrottle/openthrottle-server-auth.md`: document Passport local strategy, OpenThrottle users table (password_hash, email uniqueness), JWT payload (sub: user.id), and that login requires a user in OpenThrottle with email and password_hash set.
...
| OpenThrottle users migration | `databases/migrations/026_create_users_table.sql`                                                                |
| New migration                | `databases/migrations/031_add_users_password_hash.sql` (or next number)                                          |
| User entity                  | `packages/openthrottle/nestjs-repositories/src/modules/users/user.entity.ts`                                     |
| UsersService                 | `packages/openthrottle/nestjs-repositories/src/modules/users/users.service.ts`                                   |
