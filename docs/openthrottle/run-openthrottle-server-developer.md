...
- **Hosted chat models:** Not required to boot anything; set **`OPENROUTER_API_KEY`** to offer OpenRouter's catalog in the composer alongside locally-discovered endpoints. Unset, the provider simply cannot serve a turn. Server-side only — it never reaches the browser. See [local-quickstart.md § Chat models](./local-quickstart.md).
...
`docker compose up` applies the schema (the `migrations` service) but never seeds the login user or the service-account bearer tokens — that is a deliberate, manual one-shot: `docker compose run --rm bootstrap`. Without a host pnpm/node toolchain there is no other way to create the `developer@openthrottle.ai` account or provision `OPENTHROTTLE_MCP_AUTH_TOKEN` / `OPENTHROTTLE_WORKER_GRAPHQL_AUTH_TOKEN`. The exact copy-pasteable procedure for both a local Docker volume and a shared/real database is in [`databases/README.md` § "Bootstrapping a fully-Dockerized install"](../../databases/README.md#bootstrapping-a-fully-dockerized-install-default-user--service-accounts).
