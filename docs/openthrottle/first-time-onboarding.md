...
| **`MCP_DEVELOPER_AUTH_TOKEN`** (and server embedding config) | Auth: [AUTH.md](../../packages/mcp-developer/docs/AUTH.md). Embeddings: **`OPENAI_API_KEY`** or **`OLLAMA_*`** on **`applications/openthrottle-server/.env`** — `scripts/run-mcp-developer.sh` does not require a root OpenAI key; Ollama-only: [run-locally-oss.md](./run-locally-oss.md). |
