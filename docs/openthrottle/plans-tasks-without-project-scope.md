...
- **Set project when:** Plan or task is clearly scoped to **one** NX project (app or package), e.g. `openthrottle-developer`, `openthrottle-server`, `OpenThrottle`, `@openthrottle/openthrottle-mcp`, `@openthrottle/react-router-shadcn`. Use the NX project name from the project graph.
