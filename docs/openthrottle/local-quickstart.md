...
**Defaults:** GraphQL at **`http://localhost:6021/graphql`**, health at **`http://localhost:6021/health`**.

Quick checks:

```bash
curl -sf http://localhost:6021/health
curl -sf -X POST http://localhost:6021/graphql \
  -H 'Content-Type: application/json' \
  -d '{"query":"query { serverHealth { api database redis websocket } }"}'
```

**Optional developer UI** (separate terminal):

```bash
pnpm nx run openthrottle-developer:codegen-graphql   # if types missing after clone
