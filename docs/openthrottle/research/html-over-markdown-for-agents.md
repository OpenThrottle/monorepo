...
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Activity-by-date payloads                     | `applications/openthrottle-server/src/graphql/activity/activity.resolver.ts` (raw SQL over `plan_output_stream`, `commit_links`, tasks); MCP `get_activity_by_date` in `packages/openthrottle-mcp` / `packages/ai-mcp`                                                                                              | Agent (MCP); humans rarely raw                                   | Returns text snippets from plan output chunks among other sources; format is opaque/plain, often markdown-like in practice.                                                                                                                                                                                                                                                                                       |
| Agent Cursor skills (Markdown files)          | `.cursor/skills/agents-ralph/SKILL.md`, `.cursor/skills/agents-code-review/SKILL.md`; `.opencode/agents/*.md`                                                                                                                                                                                                       | Both                                                             | Human-authored procedure text; agents consume when the workflow or user attaches these instructions.                                                                                                                                                                                                                                                                                                              |
...
| Documentation MCP                             | `openthrottle-mcp` `semantic_search` / `get_document` over the `documentation` source (the standalone `docs-mcp` server is retired — see [mcp-registration.md § Current state](../mcp-registration.md#current-state))                                                                                               | Agent                                                            | Returns chunk `content` as text for the model; no HTML rendering layer in MCP.                                                                                                                                                                                                                                                                                                                                    |
...
| Plan and task text fields                     | Postgres `plans` (`description`, `summary`, …), `tasks` (`description`, …); GraphQL + MCP CRUD (`openthrottle-mcp`, `ai-mcp`)                                                                                                                                                                                       | Both                                                             | Unstructured `TEXT`; product and OT rules assume markdown-style authoring for long PRDs. `formatPlanAndTasksForPrompt` flattens task `description` newlines to spaces for the injected block.                                                                                                                                                                                                                     |
| Plan output stream                            | `plan_output_stream` table; `append_plan_output` / `get_plan_output` (`packages/openthrottle-mcp/src/tools/output.ts`, `packages/ai-mcp/src/tools/output.ts`); `tools/workflows/src/utils/openthrottle-ralph.ts` (`INSERT INTO plan_output_stream`); processors in `applications/openthrottle-server/src/queues/**` | Both                                                             | Append-only log of agent/worker stdout-style chunks; often markdown sections and code fences. CLI Ralph logs to terminal unless `streamToOpenThrottle` (see workflows README / `child-job`).                                                                                                                                                                                                                      |
| Ralph CLI injected prompt block               | `tools/workflows/src/utils/openthrottle-ralph.ts` (`formatPlanAndTasksForPrompt`); consumed in `tools/workflows/src/bin/ralph.ts` (`basePrompt`); executed via `tools/workflows/src/bin/run-iteration.ts` (`agentPrompt` → `cursor-agent` / `claude`)                                                               | Agent                                                            | Plain-text block labeled “OpenThrottle plan (injected by Ralph from Postgres)”; not Markdown syntax per se but same family as prompt markdown.                                                                                                                                                                                                                                                                    |
...
| openthrottle-mcp tool text results            | `packages/openthrottle-mcp` resolvers (e.g. `get_document`, `semantic_search`, plan/task getters returning `description`)                                                                                                                                                                                           | Agent                                                            | JSON/text over MCP; clients render as plain text or their own markdown UI.                                                                                                                                                                                                                                                                                                                                        |
...
| OpenThrottle custom prompts                   | `neutral` | Monaco `language="markdown"` fits human editing; if the same blob is injected wholesale into agents, heavy HTML tends toward **hurts** on tokens—treat rich formatting as optional, not the default transport.                                                                                             |
...
| Ralph CLI injected prompt block               | `hurts`   | Injected OpenThrottle blocks should stay dense plain text: HTML tags and attributes burn context without improving model reasoning for task completion markers.                                                                                                                                            |
...
| openthrottle-mcp tool text results            | `hurts`   | JSON/text over MCP is consumed by agents and thin clients; HTML payloads increase tokens and XSS surface without a universal renderer contract.                                                                                                                                                            |
...
| Field                | Detail                                                                                                                                                                                                                                                        |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Goal**             | Test whether **sanitized HTML rendering** of the same logical content (tables for task list, anchors per chunk) improves scanability without changing what gets stored in OpenThrottle.                                                                       |
...
| **Surface**          | Offline-generated `index.html` + JSON data file from `database:import-docs` output or a one-plan export; not wired into `openthrottle-mcp` responses initially.                                                                                                        |
...
**Recommended next step:** Instrument or manually count tokens (same content, three formats) for a **representative OpenThrottle injection** (`formatPlanAndTasksForPrompt` + typical `description` length). Set a **budget guideline** (e.g. “no raw HTML in injected block unless under N tokens”) for any future template change.
...
**Question:** Should `openthrottle-mcp` ever return **HTML strings** in tool results, or must MCP remain **plain text / JSON** with HTML only in apps or static files? Do Cursor and other MCP clients strip tags, double-encode, or pass through to a markdown renderer that breaks on tags?
...
**Question:** Will humans author OpenThrottle `description` / custom prompts primarily in Markdown with occasional HTML, or will we introduce WYSIWYG / split preview? How do agents **author** valid HTML without breaking fences and `<ralph:*>` markers?
...
- **Hard preconditions:**
  ...
  - **Open question (b) resolved:** token-budget guideline written for any HTML in injected OpenThrottle blocks.
...
- **Plan: "Token-cost guideline for OpenThrottle injected blocks (open question b)"** — manual / instrumented count for `formatPlanAndTasksForPrompt` with realistic descriptions; output is a budget rule.
