...
**Question:** Should `openthrottle-mcp` or `docs-mcp` ever return **HTML strings** in tool results, or must MCP remain **plain text / JSON** with HTML only in apps or static files? Do Cursor and other MCP clients strip tags, double-encode, or pass through to a markdown renderer that breaks on tags?
