...
| NestjsAuth pkg | `packages/mattscholta/nestjs-auth/` (nestjs-auth.module.ts, strategies/jwt.strategy.ts, guards/jwt-auth.guard.ts, auth.options.ts)                                                                                                                                          |
...
**Current state:** A **global JWT guard** is applied: `GlobalJwtAuthGuard` is registered as `APP_GUARD` in `app.module.ts`. It checks for `@Public()` (via `IS_PUBLIC_KEY`) on the handler or class; when set, it skips JWT and allows the request. Otherwise it delegates to `JwtAuthGuard`, so all resolvers and REST endpoints require a valid JWT by default.
...
**Roles and permissions (DB-backed):** Roles and permissions are stored in Cortex (`permissions`, `roles`, `role_permissions`, `user_roles`). Migration `034_create_roles_and_permissions_tables.sql` seeds default permissions (`settings:read`, `settings:write`, `users:read`, `users:write`) and roles (`admin`, `user`, `viewer`) with the same mapping as `@openthrottle/nestjs-rbac`. The users and roles GraphQL resolvers are protected by `GqlPermissionsGuard`, which checks permissions via `RolesService.getPermissionsForUser(userId)`. **Bootstrap:** After running migrations, no user has any role. To grant admin access, assign the `admin` role to a user (e.g. via SQL: `INSERT INTO user_roles (user_id, role_id) SELECT u.id, r.id FROM users u, roles r WHERE u.email = 'your@email.com' AND r.name = 'admin';`), or use the admin UI once the first admin is assigned by a DB admin.
