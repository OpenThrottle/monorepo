...
- **Prompt end-of-options.** The spawn argv inserts `--` immediately before the prompt so a persona system prompt that starts with YAML frontmatter (`---`) is never parsed as a CLI flag (`error: unknown option '---'` → empty turn).
