...
**Run transcripts on disk:** OpenThrottle’s API workers can mirror Ralph `stdout`/`stderr` to per-job JSONL when `BULLMQ_RUN_OUTPUT_DIR` is set (see `packages/nestjs-logging` README, section “BullMQ per-job run transcripts”, and [bullmq-run-output-spec.md](../../packages/nestjs-logging/docs/bullmq-run-output-spec.md)).
