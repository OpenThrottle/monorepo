...
| `d31f22cb-5fde-41bd-997c-5fe201ac0712` | Fix workflow-ralph debug/verbose flags and cross-repo plan lookup          | Completed   |
...
| ---------- | -------------------------------------- | ---------------------------------- | ----------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------- |
| 2026-05-29 | `d31f22cb-5fde-41bd-997c-5fe201ac0712` | CLI, foreign cwd (`shiftsmart`)    | Unknown `--debug`/`--verbose`; Plan not found with foreign workspace    | **Fixed + verified:** `--debug`/`--verbose` parse on CLI and enqueue (`ralphDebugCli` lowercase). Manual E2E: `OPENTHROTTLE_POSTGRES_URL` + plan `3fa7442e-b80d-4d64-a20f-5580fed58932` from `/Users/matt/Development/shiftsmart` — `getPlanById` succeeds; stderr shows `[workflow-ralph:ot-diagnostics]` (`postgresIdentity`: `openthrottle_user@localhost:6010/openthrottle`) and `[workflow-ralph:debug]` iteration lines. Docs: `tools/workflows/README.md`, `docs/cross-repo-usage.md`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | —           |
...
3. **Postgres identity:** Nested child `postgresIdentity` matches the OpenThrottle DB where the plan lives (use `WORKFLOW_RALPH_OT_DIAGNOSTICS=1` on the child and `OPENTHROTTLE_PLANS_SPAWN_DIAGNOSTICS=1` on the server when debugging).
...
- `Plan not found` when the plan exists in OpenThrottle (wrong DB string in child — should be ruled out by canonical URL injection).

## CLI vs queue spawn (code diff)

Use this section when choosing an auth strategy (task `2af0a6ce-0072-4319-8cde-dc4033322ea8`): the queue does **not** sandbox the child’s environment beyond OpenThrottle DB alignment.

### Environment (`buildWorkflowRalphSpawnEnv`)

- **Implementation:** `packages/ai-mcp/src/config.ts` — `{ ...workerEnv, OPENTHROTTLE_POSTGRES_URL: conn, POSTGRES_URL: conn }` when a canonical connection string is known; otherwise starts from `workerEnv`. Optional worker-set **`WORKFLOW_RALPH_SPAWN_HOME`** / **`WORKFLOW_RALPH_SPAWN_XDG_CONFIG_HOME`** override nested `HOME` / `XDG_CONFIG_HOME` (for bind-mounted Claude config). See `tools/workflows/README.md` (queue + Claude section).
...
| **Plans queue — legacy** (`processInProcessCwd`) | `pnpm` + `['exec','workflow-ralph','--plan',…]`    | `job.data.workingDirectory ?? WORKSPACE_ROOT ?? process.cwd()` | `buildWorkflowRalphSpawnEnv(process.env, { canonicalPostgresUrl: getPostgresUrl() })` — see `applications/openthrottle-server/src/queues/plans/plans.processor.ts` |
| **Plans queue — worktree**                       | `@tools/workflows` `runChildJob` → `runRalphAsync` | `handoff.worktreePath`                                         | Same helper from `process.env` + optional `canonicalPostgresUrl` from `getPostgresUrl()` on the processor — see `tools/workflows/src/utils/child-job.ts`           |
