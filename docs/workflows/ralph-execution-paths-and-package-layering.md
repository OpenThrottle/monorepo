...
> (the single-flag rollback, resolved by `resolveDefaultPlanRunKind()`). Queued runs dispatch
> **through the `AgenticWorkflowBase` pattern**: `@openthrottle/nestjs-agentic-workflow` defines the
> workflow-agnostic base (stable `id` + `createOrchestrator()`) and an `AgenticWorkflowRegistry`
> (token `AGENTIC_WORKFLOW_REGISTRY`); **`AgenticWorkflowRalph`** (id `'ralph'`) is the first concrete
> implementation wrapping `createWorkflowRalphOrchestrator`. The processor / `AgenticRalphOrchestratorService`
> resolve **by id** from the registry (behavior-neutral indirection), leaving room for `AgenticWorkflow<X>`
