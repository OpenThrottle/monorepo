...
/** `mm:ss` to seconds. Beat and cue labels are both written that way. */
export const beatSeconds = (label: string): number => {
...
/**
 * Which beat a narration cue belongs to.
 *
 * The cue's own time decides, against the episode's beat times: the beat it falls
 * in is the last one that has started. A cue at 0:11 belongs to the beat that
 * began at 0:09, whether or not any other variant put a cue at 0:09.
 *
 * Returns -1 for a cue that starts before the first beat, which is a script error
 * rather than something to silently absorb.
 *
 * This replaces POSITIONAL matching — Nth narration beat to Nth flow beat — and
 * the replacement is not cosmetic. Under the old rule a flow beat the script had
 * no line for did not go unnarrated: it consumed the slot, and every beat after it
 * inherited the previous beat's line. Video 05 shipped a take where "and there it
 * is, in the dashboard" played five seconds before the dashboard appeared. Cue
 * times cannot drift that way, because a cue names when it speaks.
 *
 * @public
 */
export const beatIndexForCue = (
  beatTimes: readonly string[],
  cueTime: string,
): number => {
  const cue = beatSeconds(cueTime);
  let found = -1;

  for (const [index, beat] of beatTimes.entries()) {
    if (beatSeconds(beat) <= cue) {
      found = index;
      continue;
    }
 ...
 *
 * `beatTimes` are the episode's beat labels in order, and the manifest's spans are
 * the same beats as actually captured — the flow transcribes the beat list, so the
 * two are index-aligned and a length mismatch is a structural error worth naming.
 * Cues then land on beats by time rather than by position.
 */
export const planTimeline = (
  ...
  if (beatTimes.length > 0 && spans.length !== beatTimes.length) {
    console.warn(
      `timeline: ${manifest.flowId} captured ${String(spans.length)} beat(s) but its episode declares ${String(beatTimes.length)}.\n` +
        `          The flow transcribes the beat list, so these must agree; cue-to-beat\n` +
        `          mapping past the mismatch is guesswork. Captured: ${spans.map((span) => span.beat).join(', ')}`,
    );
  }

  // Audio per beat INDEX, resolved from each cue's own time.
  const audioByIndex = new Map<number, number>();

  for (const segment of timings.segments) {
    const index =
      beatTimes.length === 0 ? -1 : beatIndexForCue(beatTimes, segment.beat);

    if (index < 0) {
      console.warn(
        `timeline: cue at ${segment.beat} starts before the first beat — it has no beat to sit in and will not be held for.`,
      );
      continue;
    }
    ...
    audioByIndex.set(
      index,
      (audioByIndex.get(index) ?? 0) + segment.durationSeconds,
    );
  }
  ...
  for (const [index, span] of spans.entries()) {
    const audioSeconds = audioByIndex.get(index) ?? 0;
