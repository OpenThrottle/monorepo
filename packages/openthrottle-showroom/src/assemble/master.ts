...
export interface MasterRequest {
  ...
  /**
   * The episode's beat labels in order. Cues land on beats by time against these,
   * which is what replaced positional narration matching.
   *
   * Note `variant` below is the FORMAT variant (portrait vs landscape), not the
   * narration variant — the two senses of the word meet in this file.
   */
  readonly beatTimes: readonly string[];
...
export const buildMaster = async (
  ...
  const { beatTimes, format, manifest, timings, variant, workDir } = request;
  const tag = `${String(variant.width)}x${String(variant.height)}`;

  const plans = planTimeline(manifest, timings, beatTimes);
  const cues = buildCues(timings, plans, beatTimes);
