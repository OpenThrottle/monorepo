...
- `nestjs-*` is built, always. Every `technology:nestjs` package points `main`/`module`/`types` and every `exports` condition at `dist/` and declares live `build` and `dev` targets (`dev` because `openthrottle-server:dev` cascades `^dev` — without it the package's `dist/` goes stale under you) — enforced by `scripts/check-package-entrypoints.ts`, keyed on the tag rather than on whether the package contains decorators. Do not move one to source-first because it looks clean; the whole tag flips at once when NestJS supports ESM. Reasoning: [docs/monorepo/source-first-packages-and-strip-only.md](../docs/monorepo/source-first-packages-and-strip-only.md#technologynestjs-is-built-as-a-blanket-rule).
