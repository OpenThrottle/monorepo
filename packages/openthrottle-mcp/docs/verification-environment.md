...
| **Embeddings (server)**           | `semantic_search` / ingest embed on **openthrottle-server**      | **`OPENAI_API_KEY`** or **`OLLAMA_BASE_URL`** (+ optional **`OLLAMA_EMBEDDING_MODEL`**) in **`applications/openthrottle-server/.env`** — not required by `scripts/run-openthrottle-mcp.sh`. Ollama-only: [Ollama.md § Embeddings for OpenThrottle](../../../docs/monorepo/Ollama.md#embeddings-for-openthrottle-ollama-or-openai). |
...
| -------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `semantic_search` fails or returns no results                        | Server missing **`OPENAI_API_KEY`** and **`OLLAMA_BASE_URL`**, or Ollama model dimension mismatch — see [Ollama.md § Embeddings for OpenThrottle](../../../docs/monorepo/Ollama.md#embeddings-for-openthrottle-ollama-or-openai) and `databases/README.md`. |
...
**Ollama-only:** set **`OLLAMA_BASE_URL`** (and optional **`OLLAMA_EMBEDDING_MODEL`**) in **`applications/openthrottle-server/.env`**, run Ollama, then use the launcher as above. See [Ollama.md § MCP launcher and embeddings](../../../docs/monorepo/Ollama.md#mcp-launcher-and-embeddings).
