...
- [src/conversation-stream.ts](src/conversation-stream.ts) — pure reducer `reduceStreamChunk` folding backend chunks into `StreamState`; `retryableIds` records messageIds whose terminal chunk was a retryable timeout (see below).
- [src/components/ChatRetryNotice.tsx](src/components/ChatRetryNotice.tsx) — inline "Response timed out — Retry" affordance; `ChatThread`/`ChatDialog` render it via `canRetry`/`onRetry` (on `ChatComposerControls`).
...
- **Retryable-timeout marker + recovery.** When the server's idle-timeout backstop fires it publishes a terminal chunk whose `metadataJson` carries `{ retryable: true, timedOut: true }`; `reduceStreamChunk` reads it (via `isRetryableTerminalMetadata`) into `StreamState.retryableIds` (a subset of `completedIds`). This package only exposes the marker + the `ChatRetryNotice` UI — the actual auto-retry-once + stall watchdog is host-owned (`useAgenticChatTurn`), which drives `canRetry`/`onRetry`. Full contract: `docs/reliability/chat-idle-timeout-retry.md`.
