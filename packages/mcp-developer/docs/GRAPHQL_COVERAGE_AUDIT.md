...
Audit of operations needed for notes, plans, projects, tasks, commit, activity, output, search, and health so mcp-developer can achieve parity with ai-mcp over GraphQL only (no direct Cortex Postgres).
...
| **Projects** | — (ai-mcp has no Cortex project row CRUD tools; plans/tasks carry optional `project` / `projectId`)                                       | project(id), projects, createProject, updateProject, deleteProject(input)                                | createProject, deleteProject in mutations.graphql; getProject, getProjects in queries.graphql; **delete_project** MCP tool | MCP exposes **delete_project** only. Create/update/list projects: use GraphQL (e.g. Playground, scripts) or add tools later. Deleting a project clears `project_id` on related plans/tasks (ON DELETE SET NULL). |
...
4. **Invoke the tool:** Call MCP tool `delete_project` with input `{ "id": "<uuid>" }` (e.g. from Cursor MCP or an MCP client connected to `pnpm nx run @openthrottle/mcp-developer:serve`).
