...
const skillFile = (metadataArguments: string): string =>
  [
    ...
    'metadata:',
    `  openthrottle-arguments: '${metadataArguments}'`,
    '---',
    '',
    '# Demo',
  ].join('\n');
...
describe('parseSkillArguments', () => {
  test('parses a valid JSON-string declaration and normalizes defaults', () => {
    const args = parseSkillArguments({
      metadata: {
        'openthrottle-arguments': JSON.stringify([
  ...
  test('tolerates an already-parsed inline YAML array', () => {
    const args = parseSkillArguments({
      metadata: {
        'openthrottle-arguments': [{ name: 'target' }],
      },
    });
  ...
  test.each([
    ...
    [
      'malformed JSON',
      { metadata: { 'openthrottle-arguments': '{ not json' } },
    ],
    ['non-array JSON', { metadata: { 'openthrottle-arguments': '{"a":1}' } }],
    ['empty string', { metadata: { 'openthrottle-arguments': '' } }],
    ['all entries invalid', { metadata: { 'openthrottle-arguments': '[{}]' } }],
    ['not an object', 'nope'],
    ['null', null],
