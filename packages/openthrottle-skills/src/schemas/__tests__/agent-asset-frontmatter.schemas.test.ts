...
describe('skillFrontmatterSchema source provenance', () => {
  const base = {
    description: 'A skill.',
    name: 'a-skill',
  };

  test('accepts source: openthrottle', () => {
    const result = skillFrontmatterSchema.safeParse({
      ...base,
      source: 'openthrottle',
    });
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.source).toBe('openthrottle');
    }
  });

  test('accepts source: external with a sourceUrl', () => {
    const result = skillFrontmatterSchema.safeParse({
      ...base,
      source: 'external',
      sourceUrl: 'https://example.com/skills/a-skill',
    });

    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.sourceUrl).toBe('https://example.com/skills/a-skill');
    }
  });

  test('allows source and sourceUrl to be absent entirely', () => {
    const result = skillFrontmatterSchema.safeParse(base);
  ...
  test('rejects a source value outside the enum', () => {
    const result = skillFrontmatterSchema.safeParse({
      ...base,
  ...
  test('rejects an empty-string sourceUrl', () => {
    const result = skillFrontmatterSchema.safeParse({
      ...base,
      source: 'external',
      sourceUrl: '',
