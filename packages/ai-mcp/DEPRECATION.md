...
- **Auth and consistency:** openthrottle-mcp uses `AUTH_TOKEN` (or `MCP_DEVELOPER_AUTH_TOKEN`) for authenticated GraphQL; no `POSTGRES_*` in the MCP environment.
...
2. **Environment:** Set `AUTH_TOKEN` (or `MCP_DEVELOPER_AUTH_TOKEN`) for openthrottle-mcp. You can remove `POSTGRES_URL` / `POSTGRES_*` from the MCP run environment once you no longer use ai-mcp.
