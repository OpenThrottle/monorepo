...
- **Auth and consistency:** mcp-developer uses `CORTEX_AUTH_TOKEN` (or `MCP_DEVELOPER_AUTH_TOKEN`) for authenticated GraphQL; no `CORTEX_POSTGRES_*` in the MCP environment.
...
2. **Environment:** Set `CORTEX_AUTH_TOKEN` (or `MCP_DEVELOPER_AUTH_TOKEN`) for mcp-developer. You can remove `CORTEX_POSTGRES_URL` / `CORTEX_POSTGRES_*` from the MCP run environment once you no longer use ai-mcp.
