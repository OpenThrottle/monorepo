 ...
 * split is unambiguous. The GraphQL-shaped mappers (discovery query → options)
 * live alongside these helpers in `./chat-discovery-options` (shared by both
 * apps); this module depends only on the chat types and the presentational
 * provider-glyph resolver.
 */
