...
**Layer-1 prompt profile:** default **`--prompt`** is command-style **`/agents/ralph`** (see `.cursor/commands/agents/ralph.md`). Override with **`--prompt-file`** or **`--prompt-stdin`** when needed.
