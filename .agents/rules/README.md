...
- **Root:** [`.agents/rules/`](./)
- **Coding conventions:** [`.agents/rules/coding/`](./coding/) — TypeScript/JavaScript style, types, naming, testing hints
- **Command/context rules:** [`.agents/rules/commands/`](./commands/) — When to use OpenThrottle (OT), GitHub, agents, etc.
- **All agent/editor folders:** [docs/monorepo/agent-editor-folders.md](../../docs/monorepo/agent-editor-folders.md) — `.agents/` vs `.claude/` vs `.agents/`, duplication strategy, where to edit skills and MCP config (this README covers **rules only**).
...
- **Plans in OT only:** Plans and tasks MUST be created and managed in OpenThrottle via the OT MCP (openthrottle-mcp; `create_plan`, `create_task`). Do **not** create plans in Markdown files or under [docs](../../docs/).
- **Fail loudly when unavailable:** If the OT MCP is unavailable or plan/task creation fails, report the error clearly to the user. Do **not** silently fall back to writing a plan to a `.md` file or skipping. See `.agents/rules/commands/openthrottle.mdc` and `.agents/rules/commands/agents.mdc` for command-level rules.
