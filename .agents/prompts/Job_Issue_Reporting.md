...
- **There are no issue templates** in the org (`.github/ISSUE_TEMPLATE` does not exist). Expect unstructured reports with no reproduction steps, no version, and no environment. "Needs more information from the reporter" is a legitimate and common triage outcome — say so in the task rather than inventing the missing detail.
