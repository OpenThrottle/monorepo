  ...
  implements OnApplicationShutdown, OnModuleInit
{
  ...
  async onModuleInit(): Promise<void> {
    ...
    this.logger.info(
      `Plans worker stalled-job recovery: lockDuration=${WORKER_LOCK_DURATION_MS}ms, stalledInterval=${WORKER_STALLED_INTERVAL_MS}ms, maxStalledCount=${WORKER_MAX_STALLED_COUNT}. Jobs interrupted by restart re-enter the queue within ~${(WORKER_LOCK_DURATION_MS + WORKER_STALLED_INTERVAL_MS) / 1000}s.`,
      WorkflowProcessor.name,
    );
  ...
  async process(job: WorkflowJob): Promise<WorkflowJobResult> {
    ...
    const queueName = WORKFLOW_NAME;

    try {
      const logContext = `${WorkflowProcessor.name} [planId=${planId}, jobId=${jobId}]`;
