...
export const DEMO_PLANS: readonly DemoPlan[] = [
  {
    ...
    createdAtOffset: -95,
    description:
      'The public API has no per-client limits, so one noisy integration can starve everyone else. Add a token bucket at the gateway, keyed by API key, with limits configurable per plan tier.\n\nOut of scope: billing for overages.',
    id: id('01'),
    projectId: id('a1'),
    status: 'IN_PROGRESS',
    summary:
      'Protect the public endpoints from bursts with a per-key token bucket.',
    tasks: [
      {
        ...
        createdAtOffset: -94,
        description:
          'A reusable token bucket with a configurable rate and burst size, plus tests for the refill edges.',
        id: id('0101'),
        sortOrder: 1000,
        status: 'COMPLETED',
      ...
      },
      {
        ...
        createdAtOffset: -94,
        description:
          'Read the API key from the request, look up its tier, and apply the matching bucket before the handler runs.',
        id: id('0102'),
        sortOrder: 2000,
        status: 'IN_PROGRESS',
      ...
      },
      {
        ...
        createdAtOffset: -94,
        description:
          'Return 429 with Retry-After and a machine-readable body, and document the shape.',
        id: id('0103'),
        sortOrder: 3000,
        status: 'PENDING',
  ...
  },
  {
    ...
    description:
      'Tile requests near the antimeridian return an empty result because the bounding box is normalised before the wrap is applied. Fix the order and add a regression case at longitude 180.',
    id: id('02'),
    projectId: id('a1'),
    status: 'COMPLETED',
    ...
    tasks: [
      {
        ...
        description:
          'A failing test at longitude 180 before any fix, so the fix is provably the fix.',
        id: id('0201'),
        sortOrder: 1000,
        status: 'COMPLETED',
      ...
      },
      {
        ...
        createdAtOffset: -2890,
        description:
          'Apply the wrap before normalising, and keep the normalisation for every other case.',
        id: id('0202'),
        sortOrder: 2000,
        status: 'COMPLETED',
  ...
  },
  {
    ...
    createdAtOffset: -240,
    description:
      'The dashboard bundle is 1.8 MB and the map library is half of it. Split the map route so the landing page stops paying for a component most visitors never reach.',
    id: id('03'),
    projectId: id('a2'),
    status: 'QUEUED',
    ...
    tasks: [
      {
        ...
        createdAtOffset: -239,
        description:
          'Record the current bundle composition so the improvement is a number, not an impression.',
        id: id('0301'),
        sortOrder: 1000,
        status: 'PENDING',
      ...
      },
      {
        ...
        createdAtOffset: -239,
        description:
          'Lazy-load the map route and confirm the landing page no longer pulls the map library.',
        id: id('0302'),
        sortOrder: 2000,
        status: 'PENDING',
  ...
  },
  {
    ...
    description:
      'Support saved views so a user can return to a bounding box, zoom level and layer set without rebuilding it by hand every session.',
    id: id('04'),
    projectId: id('a2'),
    status: 'PENDING',
    ...
    tasks: [
      {
        ...
        description:
          'Decide what a view actually contains, and what happens when a layer it references is deleted.',
        id: id('0401'),
        sortOrder: 1000,
        status: 'PENDING',
  ...
  },
  {
    ...
    description:
      'Move the tile cache off the app nodes and onto a shared store so a deploy stops cold-starting every client.',
    id: id('05'),
    projectId: id('a1'),
    status: 'COMPLETED',
    ...
    tasks: [
      {
        ...
        description:
          'Provision the shared store and prove a warm read path end to end.',
        id: id('0501'),
        sortOrder: 1000,
        status: 'COMPLETED',
  ...
  },
  {
    ...
    createdAtOffset: -600,
    description:
      'The quickstart still describes the old auth header. Rewrite it against the current API and add a working example request.',
    id: id('06'),
    projectId: id('a1'),
    status: 'PENDING',
    ...
    tasks: [
      {
        ...
        createdAtOffset: -590,
        description:
          'Replace the stale header example and verify every request in the page against a live endpoint.',
        id: id('0601'),
        sortOrder: 1000,
        status: 'PENDING',
  ...
  },
  {
    ...
    createdAtOffset: -30,
    description:
      'Expose the layer catalogue over the API so integrations can discover layers instead of hard-coding names.',
    id: id('07'),
    projectId: id('a1'),
    status: 'PENDING',
    ...
    tasks: [
      {
        ...
        createdAtOffset: -29,
        description:
          'Return the catalogue with stable ids, so a rename does not break a caller.',
        id: id('0701'),
        sortOrder: 1000,
        status: 'PENDING',
  ...
  },
  {
    ...
    description:
      'Two clients reported duplicate webhook deliveries. Suspected retry without an idempotency key. Needs a reproduction before anything is changed.',
    id: id('08'),
    projectId: id('a1'),
    status: 'BLOCKED',
    ...
    tasks: [
      {
        ...
        description:
          'Blocked: needs delivery logs from the affected window, which are not retained that far back.',
        id: id('0801'),
        sortOrder: 1000,
        status: 'BLOCKED',
  ...
  },
  {
    ...
    description:
      'Upgrade the tile renderer to the current major and drop the two patches we were carrying.',
    id: id('09'),
    projectId: id('a1'),
    status: 'COMPLETED',
    ...
    tasks: [
      {
        ...
        description:
          'Upgrade, then remove both local patches and confirm rendering is unchanged.',
        id: id('0901'),
        sortOrder: 1000,
        status: 'COMPLETED',
  ...
  },
  {
    ...
    description:
      'Cancelled in favour of the saved-views plan, which covers the same need without a second concept.',
    id: id('10'),
    projectId: id('a2'),
    status: 'CANCELED',
...
export const DEMO_RUN = {
  ...
  chunks: [
    ...
    { content: 'Reading src/gateway/throttle/bucket.ts', offset: -17.0 },
    {
    ...
    { content: '$ pnpm test --filter atlas-api throttle', offset: -14.0 },
    {
      content:
        'FAIL bucket.test.ts > refills to exactly the burst size\n  expected 10 received 11',
      offset: -13.0,
    },
    { content: 'Off by one on the refill clamp. Fixing.', offset: -12.0 },
    {
      content: 'Writing src/gateway/throttle/bucket.ts (+2 -2)',
      offset: -11.0,
    },
    { content: '$ pnpm test --filter atlas-api throttle', offset: -10.0 },
    { content: 'PASS bucket.test.ts (7 tests)', offset: -9.0 },
    { content: '$ pnpm lint --filter atlas-api  -> clean', offset: -8.0 },
    {
    ...
    {
      content:
        'Committed 4f2a1c8 feat(atlas-api): add a token bucket for per-key rate limiting',
      offset: -6.0,
    },
    {
    ...
    { content: 'Reading src/gateway/middleware/index.ts', offset: -4.0 },
    {
      content: 'Writing src/gateway/middleware/throttle.ts (+48 -0)',
      offset: -3.0,
    },
    { content: '$ pnpm test --filter atlas-api gateway', offset: -2.0 },
    { content: 'PASS gateway middleware (12 tests)', offset: -1.0 },
  ],
