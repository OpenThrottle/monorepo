...
**Source (provenance):** SKILL.md frontmatter may declare `source: openthrottle` (authored and managed in this monorepo) or `source: external` with an optional `sourceUrl` origin link. An omitted or unrecognized value always reads as `external` — only explicitly claimed skills show the OpenThrottle badge. The index table surfaces this as a Source column plus an All / OpenThrottle / External toolbar filter; ingested `projectSkills` rows overlay the disk value when present.
