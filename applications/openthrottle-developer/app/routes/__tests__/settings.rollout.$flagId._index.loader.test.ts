...
const { loader } = await import('../settings.rollout.$flagId._index');

const mockExecute = vi.mocked(executeGraphqlWithAuth);

const buildArgs = (flagId: string): Route.LoaderArgs => {
  const request = new Request(`http://localhost/settings/rollout/${flagId}`);
  return {
    context: createTestRouterContext(),
    params: { flagId },
    pattern: '/settings/rollout/:flagId',
    request,
...
describe('routes/settings.rollout.$flagId._index loader', () => {
  beforeEach(() => {
