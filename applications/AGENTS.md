...
- Three React Router v8 + Vite apps: `openthrottle-developer`, `openthrottle-admin`, `openthrottle-website`.
...
- Layout: `app/routes/` route modules (registered in `app/routes.ts`) stay thin and delegate to `app/routing/<area>/` feature folders (`components/`, `data/`, `hooks/`, `utils/`), imported via the `~/*` → `app/*` tsconfig alias. Component vs data file boundaries: [`docs/monorepo/code-style.md`](../docs/monorepo/code-style.md#component-and-data-boundaries).
