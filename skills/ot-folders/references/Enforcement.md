...
| `check-package-entrypoints`              | package `main`/`types`/`exports`, plus live `build` and `dev` targets on every `technology:nestjs` package | **error**                                 | shrink-only baseline + an empty `NESTJS_BLOCKERS` map, both of which error when stale | `pnpm run check:local:package-entrypoints`     | `docs/monorepo/source-first-packages-and-strip-only.md` |
