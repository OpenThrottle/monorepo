...
#               server /health 200 + developer / 200.
#   dev       — dev profile: docker compose --profile dev watch (root), assert
#               server /health 200 + developer / 200 against the dev images.
...
wait_http() {
  local url="$1" want="$2" label="$3" tries="${4:-60}"
...
smoke_prod() {
  ...
  wait_http "http://localhost:${DEVELOPER_PORT}/" 200 "developer /"
...
smoke_dev() {
  ...
  wait_http "http://localhost:${DEVELOPER_PORT}/" 200 "developer-dev /" 60
