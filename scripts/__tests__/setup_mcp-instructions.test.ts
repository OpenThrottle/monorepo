 ...
 * `docs/marketing/scripts/05-connect-ot-mcp.md`). It cannot import this script — the
 * developer app's tsconfig owns its own file list and a root script is not in it — so
 * it reads a committed capture of the output instead, and this test is what stops
 * that capture from drifting away from the renderer.
 *
...
describe('the screencast capture of the printed block', () => {
  ...
  it('still matches what the renderer produces', () => {
    ...
    expect(
      ...
      "the recorded MCP instructions are stale — regenerate the capture at applications/openthrottle-developer/tests/demo/surfaces/mcp-instructions.txt from renderInstructions('/workspace/openthrottle', { claude: false, cursor: false })",
    ).toBe(expected);
  });
