...
echo ""
echo "For local embeddings (node-client, openthrottle:import): set OLLAMA_BASE_URL (default http://localhost:11434)"
...
# Embedding models (used when OLLAMA_EMBEDDING_MODEL / OLLAMA_BASE_URL are set for node-client and openthrottle:import).
# For OpenThrottle schema (vector 1536), use a model that outputs 1536 dimensions; these models use other dimensions (all-minilm 384, mxbai-embed-large 1024, nomic-embed-text 768). For 1536-dim OpenThrottle embeddings use OpenAI or an Ollama model that outputs 1536 when available. See databases/README.md § Embedding dimension strategy.
