...
Maps ai-mcp and Cortex app query patterns to existing indexes and identifies gaps. Used to add migrations (see `migrations/`) and the Indexes subsection in README.

## Query patterns (source: ai-mcp cortex-client, Cortex app)

| Query / feature              | Table(s)                         | Filter / sort                                                                                  | Used by                             |
| ---------------------------- | -------------------------------- | ---------------------------------------------------------------------------------------------- | ----------------------------------- |
| listPlansByStatus            | plans                            | status, assignee (author OR assignee), title ILIKE; ORDER BY created_at or updated_at ASC/DESC | MCP list_plans_by_status, Cortex UI |
...
4. Document all indexes and when to add new ones in README (see task “Document Cortex index strategy in README”).
