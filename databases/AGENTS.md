...
- Every `CREATE TABLE` must have `COMMENT ON TABLE` in the **same** migration file (tone model: `migrations/038_create_plan_runs_table.sql`; say "OpenThrottle", not "OpenThrottle"). `COMMENT ON COLUMN` for non-obvious columns. Enforced diff-scoped vs `main` by `pnpm nx run monorepo:check-migration-table-comments` (also in `check:local`).
