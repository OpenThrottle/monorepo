-- Batch A (plan c8be6591-7314-4b43-aaa4-6e4b7ab59e59): COMMENT ON TABLE for Cortex tables (≤10 per batch).
-- Tone matches databases/migrations/038_create_plan_runs_table.sql (short purpose-focused prose).

COMMENT ON TABLE commit_links IS 'Links git commits in a repository to Cortex plans and optionally to a specific task for traceability.';
...
COMMENT ON TABLE daily_stats IS 'One UTC calendar day of aggregated Cortex activity counts and status rollups for dashboards and reporting.';
...
COMMENT ON TABLE notes IS 'Quick unstructured notes from the Cortex MCP for capture separate from structured plans and tasks.';
