...
export interface NestJSQueueGeneratorSchema {
  readonly application?: string;
  readonly interactive?: boolean;
  readonly name?: string;
}

export const generatorNestJSQueue = async (
  tree: Tree,
  schema: NestJSQueueGeneratorSchema,
): Promise<void> => {
  ...
  const source = join(__dirname, `files/queue`);
  const destination = `applications/${application}/src/queues/`;
  ...
  // ✅ Success
  const message = `\n✅ NestJS Queue "${name}" created in "${application}"!\n`;
  logger.info(message);
};
