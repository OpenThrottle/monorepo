...
export interface NestJSSimpleServiceGeneratorSchema {
  readonly application?: string;
  readonly interactive?: boolean;
  readonly name?: string;
}

export const generatorNestJSSimpleService = async (
  tree: Tree,
  schema: NestJSSimpleServiceGeneratorSchema,
): Promise<void> => {
  ...
  const source = join(__dirname, `files/simple-service`);
