...
This document describes how verification (lint, test, typecheck) is integrated with spawned Ralph jobs, where it runs in the workflow, and how results are reported back to the API or Cortex.
...
- **Single flow:** When the API (or BullMQ processor) runs a Ralph job, it calls `runWorktreeWorkflow` once. That workflow: acquire → run loop (e.g. `runChildJob` spawning `pnpm exec workflow-ralph`) → **ensure commit** (clean check + optional lint/test/typecheck) → release. Verification runs **inside** this flow, after the loop succeeds and before the worktree is released.
...
- **Configuration:** The caller passes `WorktreeWorkflowOptions.ensureCommit` (`ParentJobEnsureCommitOptions`): `runChecks` (default `true`) to enable or disable lint/test/typecheck, optional `base` for nx affected, and optionally `timeoutMs`, `signal` (AbortSignal), and `onChunk` for progress/cancel during nx checks (spawn + Promise). The spawned job is bound to one worktree; all verification runs in that worktree’s `cwd`.
...
3. **Ensure commit** — Only when the loop succeeded: ensure working tree is clean and (optionally) run lint, test, and typecheck.
...
- **Checks (when `runChecks: true`, default):** Run in order via `pnpm exec nx` (spawn + Promise; optional timeout and AbortSignal):
  - `lint` (either `nx affected -t lint --base <base>` or `nx run-many -t lint`)
  - `test`
  - `typecheck`
...
- **Check failed:** `{ ok: false, reason: 'checks_failed', check: 'lint' | 'test' | 'typecheck', stderr?: string, stdout?: string }`
...
| **What runs**       | Working tree clean check; then (if `runChecks: true`) lint → test → typecheck via nx. |
