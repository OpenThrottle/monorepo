...
- **Local CLI** and **legacy spawn** are the **Postgres-direct** lineage (this package, `@tools/workflows`); spawn always bottoms out in the Local CLI child.
...
- **Single health check:** only the orchestrator does a `getServerHealth` GraphQL preflight; Local CLI / legacy spawn use a Postgres TCP check (`ensureDatabaseReachableOrExit`). See [getServerHealth vs workflow GraphQL transport errors](#getserverhealth-vs-workflow-graphql-transport-errors-ralph-startup).
