...
> **DEPRECATED.** This package is being retired in favor of **`@openthrottle/nestjs-agentic-workflow`**
> (Nest DI + BullMQ agentic workflows), **`@openthrottle/nestjs-worktrees`** (worktree acquire/run/release),
> and **`@openthrottle/openthrottle-agentic-workflow`** (transport-free orchestrator contracts).
> Tracked by OpenThrottle plan `62c58119-54c3-4b3e-b57c-bdf921e247cc`. Exports remain available as
> **re-export shims** until all consumers migrate; do not add new imports from this package.
...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pnpm exec workflow-ralph --plan/--task …` (human, copied-from-UI, or nested child of spawn)                                                                                  | **Local CLI**                                                        | The `workflow-ralph` Node process itself                                                          | `src/bin/ralph.ts` → `main()`                                                                                                                                         | **GraphQL** (`executeWorkflowGraphqlV2`; rollback: `WORKFLOW_RALPH_TRANSPORT=postgres-direct`)     | **None** built in (validate manually, e.g. `pnpm run check:local`)                                                                                  |
| GraphQL **`enqueuePlanRun`** (canonical) → **default: orchestrator** (job `Agentic Ralph`); **legacy spawn** (job `run-plan`) only when `OPENTHROTTLE_DEFAULT_RUN_KIND=spawn` | **Plans queue — orchestrator (default)** / **spawn (legacy opt-in)** | `openthrottle-server` worker, **in-process** by default; spawn forks a **child** `workflow-ralph` | Default resolves **`AgenticWorkflowRalph`** via the `AGENTIC_WORKFLOW_REGISTRY` → `createWorkflowRalphOrchestrator`; spawn delegates to **Local CLI** (`runChildJob`) | **GraphQL** by default; spawn child uses GraphQL unless `WORKFLOW_RALPH_TRANSPORT=postgres-direct` | Default: **none** (no `ensureCommit`). Spawn + **`WORKTREE_TARGETS` set:** `ensureCommit { runChecks: true }` (clean tree + nx lint/test/typecheck) |
| GraphQL **`enqueuePlanRalphOrchestrator`** → BullMQ job `Agentic Ralph`                                                                                                       | **Plans queue — orchestrator**                                       | `openthrottle-server` worker, **in-process** (no child CLI)                                       | Resolves **`AgenticWorkflowRalph`** (`AGENTIC_WORKFLOW_REGISTRY`, id `'ralph'`) → `createWorkflowRalphOrchestrator` (`@openthrottle/openthrottle-agentic-ralph`)      | **GraphQL** (`executeGraphqlV2` typed documents)                                                   | **None** (no `runWorktreeWorkflow`; no parent-job `ensureCommit`)                                                                                   |

- **Orchestrator is the default for queued runs.** `enqueuePlanRun` routes to the in-process GraphQL orchestrator unless **`OPENTHROTTLE_DEFAULT_RUN_KIND=spawn`** flips it back to the nested-CLI spawn path (the single-flag rollback; the `workflowPlanRun` alias follows `enqueuePlanRun`).
- **Pattern vs workflow (Stage (a) base/concrete split).** `@openthrottle/nestjs-agentic-workflow` defines the workflow-agnostic **`AgenticWorkflowBase`** pattern (stable `id` + `createOrchestrator()`) and an **`AgenticWorkflowRegistry`** (token `AGENTIC_WORKFLOW_REGISTRY`); **`AgenticWorkflowRalph`** is the first concrete implementation (id `'ralph'`) wrapping `createWorkflowRalphOrchestrator`. The dispatcher resolves a workflow **by id** through the registry, so future `AgenticWorkflow<X>` register side-by-side without changing the dispatcher. Register via `NestjsAgenticWorkflowModule.registerWorkflow`.
- **Local CLI** and **legacy spawn** use **GraphQL** by default (`executeWorkflowGraphqlV2` + codegen documents). Set **`WORKFLOW_RALPH_TRANSPORT=postgres-direct`** to roll back to the legacy `pg.Client` lineage.
- **Orchestrator** is the **GraphQL-first** lineage and the basis for the [target architecture](#target-architecture-phase-2) — it borrows only the iteration runner (`createCursorWorkflowRalphIterationRunner`) from this package, not the CLI or the worktree workflow.
- **Single health check:** Local CLI, spawn, and orchestrator use the `getServerHealth` GraphQL preflight (`ensureDatabaseReachableOrExit` → `GetServerHealthDocument`). Postgres TCP (`SELECT 1`) remains only under **`WORKFLOW_RALPH_TRANSPORT=postgres-direct`**. See [getServerHealth vs workflow GraphQL transport errors](#getserverhealth-vs-workflow-graphql-transport-errors-ralph-startup).
...
- **Deprecate `@tools/workflows`.** Retire the Postgres-direct CLI + iteration runner + worktree workflow. **`workflow-ralph` stays** as a thin local CLI shim (GraphQL transport by default; `WORKFLOW_RALPH_TRANSPORT=postgres-direct` rollback). **`workflow-link-merge`** and **`workflow-lighthouse`** stay in this package until relocated to standalone tool packages. **`workflow-nx-validate` is removed** — enforcement moves to Stage (d) after-phase hooks. **`doc-ingestion`** stays at `@tools/workflows/doc-ingestion` until separately scoped. The orchestrator path is the default for queued runs.
...
- **GraphQL-only transport, with exactly one exception:** every workflow request (plan/task fetch, status updates, plan-output streaming, enqueue, hook data) goes through GraphQL (`executeGraphqlV2` / `executeWorkflowGraphqlV2`). The **single** documented exception is a **health check** (`getServerHealth`) used as a read-before-write preflight. A Postgres-direct implementation survives behind `WORKFLOW_RALPH_TRANSPORT=postgres-direct` as an operational rollback only — never for new code. The full contract, operation enumeration, and the transport-selection rules live in [`docs/workflows/graphql-only-transport-boundary.md`](../../docs/workflows/graphql-only-transport-boundary.md).
- **Jest-style before/after workflow hooks**, scoped to the plan and its tasks (not Ralph iterations), each running as a **child BullMQ job** of the parent plan run:
  ...
  - Today's run-level `before_run` / `after_run` map to `beforeAll` / `afterAll`; see task `c8896177` for the design work.
...
- `workflow-link-merge` — record the squash commit (after PR merge) against an OpenThrottle (OT) plan as a work-ledger `git_commit` artifact; run with `--plan <uuid> --sha <squash-sha> --repo <owner/repo>` (Option A: no pre-merge linking). The `--sha` must be the **landed commit on the default branch**, not the PR head SHA. On merge-queue-protected branches, wait until the PR actually reports `mergedAt` / `mergeCommitSha` and use that value. Orchestrates the generic ledger primitives (session → subject → artifact); the legacy `link_commit`/`commit_links` path is retired. See **Recording merged commits on the work ledger (Option A workflow)** in `databases/README.md`.
...
- `workflow-memory-reconcile` — reconcile an agent memory index against the world; run with `--index <path/to/MEMORY.md> [--repo <owner/repo>]`. Extracts every PR reference and OT plan id, resolves PR state via `gh` (one `gh pr list` call, not one per reference) and plan status via OT GraphQL, and prints **only** the disagreements. Exit 0 = none, exit 1 = disagreements found. Deliberately narrow: a reference carrying no explicit state word asserts nothing and is not checked, an unresolvable reference is reported separately and never as drift, and the report ends by naming what it did not cover — a clean run says the machine-checkable claims agree, **not** that the index is accurate. When OT GraphQL is unreachable it skips the plan half and says so rather than reporting every plan as missing. Also runs, unconditionally and before any network call, the **both-directions integrity check** — every memory file reachable from the index or an archive, and every pointer resolving to a real file. Those catch different bugs: an orphan is the bulk-rewrite failure (files stay on disk, nothing errors, they stop being reachable), a broken link is the rename failure. When plan data is available it additionally prints **archive candidates** (plan COMPLETED, PRs settled, nothing owed) and the entries **held back** with the reason — demotion is always a proposal, never applied. Wired to the weekly `Job_MemoryDrift` agent job (`.agents/prompts/Job_MemoryDrift.md`).
- `workflow-lighthouse` — Lighthouse audits (independent utility; may relocate to its own tool package later).
...
- **Prompt (source of truth):** [`skills/agents-ralph/SKILL.md`](../../skills/agents-ralph/SKILL.md) (generated views: `.agents/skills/agents-ralph/`, `.claude/skills/agents-ralph/`)
...
- **Ralph `workflow-ralph` startup does not use `getServerHealth`.** It uses `ensureDatabaseReachableOrExit` in `src/utils/openthrottle-ralph.ts`: a direct Postgres TCP check with the **CLI process** `POSTGRES_*` (or URL). That is the right preflight for loading plans and tasks from OpenThrottle in this binary.
...
- **Commit as you go:** After each task (or logical chunk), commit with a conventional message and include `Plan-Id` and `Task-Id` in the commit body or footer. Push when the plan is ready for review, not after every task. Do **not** record those branch commits in OpenThrottle; record only the landed squash / queue-produced commit after PR merge via `workflow-link-merge` (see **Recording merged commits on the work ledger** in `databases/README.md`).
...
Ralph selects **one** runner implementation per process via `--backend` (default `cursor`), or `WORKFLOW_RALPH_BACKEND` / optional `backend` in `.workflow-ralph.json`, with the same precedence as other defaults (CLI > env > file > built-in). The binary stays `workflow-ralph`; backends are defined in **[`@openthrottle/openthrottle-drivers`](../../packages/openthrottle-drivers/README.md)** (one `defineDriver` module per CLI), and `src/bin/run-iteration.ts` is a thin adapter that resolves the driver from that registry (`getDriver`) and delegates to its shared execution engine. Adding a backend is a `defineDriver` module + test there, not an edit here. The full supported set is `claude`, `codex`, `cursor`, `grok`, `opencode` (see the drivers capability matrix); the table below covers the two originals.

| Backend id | Runner                                                                                                                                                                             | Layer 1 (prompt profile)                                                                                                                                                                                                                                                                                                                            |
| ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cursor`   | Cursor CLI `cursor-agent --force -p "<full prompt>"` (optional `--model`)                                                                                                          | **Named:** command-style path (e.g. `/agents/ralph`) as the start of the injected prompt string. **File / stdin:** `--prompt-file` / `--prompt-stdin` supply the same string shape (full text before plan/task injection); use a file path to reference `skills/agents-ralph/SKILL.md` (or its `.agents/skills/` view) without duplicating content. |
| `claude`   | Claude Code CLI `claude -p --permission-mode acceptEdits "<full prompt>"` (optional `--model`; omit when preset is `auto`). Requires `claude` on PATH and auth per Anthropic docs. | Same injected prompt string as `cursor`; layer 1 resolution is unchanged.                                                                                                                                                                                                                                                                           |
...
**Agent CLI worktree (`--worktree`):** Optional `-w` / `--worktree [name]` on **cursor-agent** and **claude** per iteration. Precedence: CLI → `WORKFLOW_RALPH_WORKTREE` → `.workflow-ralph.json` → BullMQ handoff `targetId` (when using `runChildJob`) → omit. Physical git worktrees (`WORKTREE_TARGETS`, `cwd`) are unchanged.
...
Ralph can be invoked from another repo by pointing at this monorepo's workflow binary. **OpenThrottle is required:** set `POSTGRES_URL` or `POSTGRES_*` in the environment (e.g. export from this monorepo's `.env` or a shared config). When the foreign repo's own `.env` would point at a different Postgres, set **`OPENTHROTTLE_POSTGRES_URL`** (or override `POSTGRES_URL`) to OpenThrottle's OpenThrottle connection string before invoking Ralph; queue spawns inject this automatically. Ralph injects plan and tasks into the prompt; **do not write a ref file**—the workflow never writes one and invokers rely on planId-in-prompt only. See [docs/cross-repo-usage.md](docs/cross-repo-usage.md) for details.
...
2. The value flows through `buildRunPlanOrchestratorJobData` into the BullMQ job payload (`RunPlanOrchestratorJobData.workingDirectory`).
3. The plans queue worker (`PlansProcessor`) uses `workingDirectory` as the orchestrator's working directory. When omitted, the worker falls back to `WORKSPACE_ROOT` or `process.cwd()` (monorepo root).
...
**OpenThrottle DB identity (fixes “Plan not found” with a foreign cwd):** The worker resolves the same OpenThrottle connection string it uses for OpenThrottle and passes it into the nested process as **`OPENTHROTTLE_POSTGRES_URL`** and **`POSTGRES_URL`**. `getPostgresConfig()` prefers `OPENTHROTTLE_POSTGRES_URL` first, so tooling in another repo (env loaders, `pnpm`, or a local `.env`) cannot point Ralph at a different Postgres than the worker. You normally do not set these manually.

**Deterministic `workflow-ralph` resolution (fixes binary not found with a foreign cwd):** `buildWorkflowRalphSpawnEnv` (`@tools/workflows`) prepends the OpenThrottle monorepo's **`node_modules/.bin`** to the spawn `PATH`, so `pnpm exec workflow-ralph` resolves this monorepo's binary regardless of `cwd` — without relying on the dev shell PATH bleeding in (which fails in clean/Docker envs). The OpenThrottle root is located, in order, from **`WORKFLOW_RALPH_OT_ROOT`** (explicit override), then `WORKSPACE_ROOT` (when it contains `pnpm-workspace.yaml`), then by walking up from the `@tools/workflows` module, then from `process.cwd()`. **Set `WORKFLOW_RALPH_OT_ROOT`** to this monorepo's absolute path when the marker file is not reachable (e.g. bundled or Docker deployments).

**Deterministic `--project` validation (fixes wrong Nx graph with a foreign cwd):** `getNxProjectNames()` (`tools/workflows/src/utils/projects.ts`) validates `--project` against the project graph. Nx resolves its graph from `process.cwd()` by default, so a foreign `workingDirectory` would otherwise yield the **target repo's** graph (or none), breaking validation and polluting logs with bare/incomplete `nx` invocations. Before building the graph, `pinNxWorkspaceRootToOpenThrottle()` pins resolution to the OpenThrottle root (same precedence as binary resolution above, via `getOpenThrottleRoot`) by setting **`NX_WORKSPACE_ROOT_PATH`**, disabling the Nx daemon (`NX_DAEMON=false`) for an in-process build, and updating Nx's cached `workspaceRoot`; all three are reverted afterward. Result: `--project` validates against the OpenThrottle graph regardless of `cwd`. Set **`WORKFLOW_RALPH_OT_ROOT`** when the root cannot be auto-detected (e.g. Docker).

**Diagnostics (compare worker vs nested CLI):** Nested `workflow-ralph` does **not** load `.env` from `workingDirectory`. Plan lookup is **Postgres** (`getPostgresConfig` in `@openthrottle/node-client`), not `API_URL_INTERNAL` / GraphQL. To verify identity when debugging “Plan not found” with a custom cwd:
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| **`enqueuePlanRun`**               | **Canonical** enqueue. **Default: orchestrator** (resolves `AgenticWorkflowRalph` via the registry, `runKind: 'orchestrator'`); routes to the **legacy spawn** path (`runKind: 'spawn'`, nested `pnpm exec workflow-ralph`) only when **`OPENTHROTTLE_DEFAULT_RUN_KIND=spawn`** | Developer app **Run plan**, scripts, MCP clients           |
| **`enqueuePlanRalphOrchestrator`** | In-process Ralph orchestrator (`runKind: 'orchestrator'`) — no nested CLI child; resolves `AgenticWorkflowRalph` by id `'ralph'` from `AGENTIC_WORKFLOW_REGISTRY`                                                                                                               | Advanced / server-side orchestrator path                   |
| **`workflowPlanRun`**              | **Deprecated alias** for `enqueuePlanRun` (same input and result, including the orchestrator-by-default routing; schema `deprecationReason` points to canonical mutation)                                                                                                       | Legacy clients only — do not use in new UI or integrations |
...
| **Plans queue — orchestrator (default)** | **No** nested `workflow-ralph` child: in-process **`createWorkflowRalphOrchestrator`**, resolved as **`AgenticWorkflowRalph`** via the `AGENTIC_WORKFLOW_REGISTRY` (id `'ralph'`); same logical loop as CLI with injected GraphQL + iteration runner | GraphQL **`enqueuePlanRun`** (default) or **`enqueuePlanRalphOrchestrator`** → job payload `runKind: 'orchestrator'` |

Implementation notes: every queued plan run is now an in-process orchestrator run (`runKind: 'orchestrator'`); the nested-`workflow-ralph` spawn worker path was removed (OT plan 2ab62876) and `enqueuePlanRun` routes through the orchestrator. The job shape is documented in `applications/openthrottle-server/src/queues/plans/plans.types.ts` (`RunPlanJobData` = `RunPlanOrchestratorJobData`). The plans processor / `AgenticRalphOrchestratorService` resolve the workflow **by id** through `AGENTIC_WORKFLOW_REGISTRY` (the workflow-agnostic `AgenticWorkflowBase` pattern) rather than constructing the Ralph orchestrator directly. The API worker wires deps via `@openthrottle/openthrottle-agentic-ralph` (`AgenticRalphOrchestratorService`, `plans.processor.ts`).

### Legacy spawn skips `ensureCommit` (configure `WORKTREE_TARGETS` for post-run checks)

GraphQL **`enqueuePlanRun`** now defaults to the **orchestrator** path; the **legacy spawn** path runs only when **`OPENTHROTTLE_DEFAULT_RUN_KIND=spawn`** (or a job payload `runKind: 'spawn'`). On that legacy spawn path, whether post-run validation runs depends on **`WORKTREE_TARGETS`** on the **openthrottle-server** worker process (see [`@openthrottle/nestjs-worktrees`](../../packages/nestjs-worktrees/README.md)):
...
- **Why it happens:** Interactive login in your own terminal stores OAuth/session data for **your** user and `HOME`. A BullMQ worker, Docker service, or `nx`/launchd job often runs as **another user**, with a **different `HOME`**, or inside a container **without** those files—so `claude -p …` exits unauthenticated even though you logged in elsewhere.
- **What OpenThrottle passes through:** `runChildJob` spawns `pnpm exec workflow-ralph` with `buildWorkflowRalphSpawnEnv` (`@tools/workflows`), which merges **OpenThrottle Postgres** (`POSTGRES_URL` / `OPENTHROTTLE_POSTGRES_URL`) and otherwise forwards the worker’s `process.env`. It does **not** inject Claude credentials; you must configure them for the worker process.
