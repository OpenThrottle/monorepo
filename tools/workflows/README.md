...
- `workflow-link-merge` — record the squash commit (after PR merge) against an OpenThrottle (OT) plan as a work-ledger `git_commit` artifact; run with `--plan <uuid> --sha <squash-sha> --repo <owner/repo>` (Option A: no pre-merge linking). Orchestrates the generic ledger primitives (session → subject → artifact); the legacy `link_commit`/`commit_links` path is retired. See **Recording merged commits on the work ledger (Option A workflow)** in `databases/README.md`.
...
- **Commit as you go:** After each task (or logical chunk), commit and push with conventional commits; include `Plan-Id` and `Task-Id` in the commit body or footer. Do **not** record those commits in OpenThrottle; record only the squash commit after PR merge via `workflow-link-merge` (see **Recording merged commits on the work ledger** in `databases/README.md`).
