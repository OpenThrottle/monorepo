...
| **Plans queue — spawn (legacy opt-in)**  | Nested `pnpm exec workflow-ralph --plan <planId>` inside the worker (`runChildJob`), optionally inside an acquired worktree when `WORKTREE_TARGETS` is set                                                                                           | GraphQL **`enqueuePlanRun`** with **`OPENTHROTTLE_DEFAULT_RUN_KIND=spawn`** → job payload `runKind: 'spawn'`         |

Implementation notes: the default `runKind` resolves via **`resolveDefaultPlanRunKind()`** (`enqueue-plan-ralph-tuning.ts`); the discriminant and argv/context mapping are documented in `applications/openthrottle-server/src/queues/plans/plans.types.ts` (`RunPlanJobData`, `runKind`). The plans processor / `AgenticRalphOrchestratorService` resolve the workflow **by id** through `AGENTIC_WORKFLOW_REGISTRY` (the workflow-agnostic `AgenticWorkflowBase` pattern) rather than constructing the Ralph orchestrator directly. The typed orchestrator package (`@openthrottle/openthrottle-workflows`) remains the portable contract; the API worker wires deps via `@openthrottle/openthrottle-agentic-ralph` (`AgenticRalphOrchestratorService`, `plans.processor.ts`).
