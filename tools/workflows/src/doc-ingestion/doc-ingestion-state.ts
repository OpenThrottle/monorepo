...
/**
 * Returns prior state for a scope: path -> { contentHash, updatedAt }. Empty Map if scope has no rows.
 */
...
/**
 * Upserts prior state for the given scope and entries. Idempotent per (scope, path).
 */
...
/**
 * Removes prior state for the given scope and paths. No-op for paths not present.
 */
...
/**
 * Returns the content hash for (scope, path), or undefined if not stored.
 */
